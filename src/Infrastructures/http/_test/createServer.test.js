...
describe('HTTP server', () => {
  ...
  it('should response 500 when authentication token manager cannot be resolved', async () => {
    const app = await createServer({
      getInstance: vi.fn().mockImplementation((dependencyName) => {
        if (dependencyName === AuthenticationTokenManager.name) {
  ...
  it('should response 500 when add thread use case fails', async () => {
    ...
    const app = await createServer({
      getInstance: vi.fn().mockImplementation((dependencyName) => {
        if (dependencyName === AuthenticationTokenManager.name) {
          return mockAuthenticationTokenManager;
        }

        if (dependencyName === AddThreadUseCase.name) {
          return {
