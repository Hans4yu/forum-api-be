...
import ThreadDetailPresenter from '../../presenters/ThreadDetailPresenter.js';
...
class ThreadsHandler {
  ...
  async getThreadDetailHandler(req, res, next) {
    try {
      ...
      res.status(200).json({
        status: 'success',
        data: {
          thread: ThreadDetailPresenter.present(thread),
        },
      });
    } catch (error) {
      next(error);
  ...
  async postCommentHandler(req, res, next) {
    try {
      ...
      const addedComment = await addCommentUseCase.execute(req.body, req.params.threadId, req.auth.id);
  ...
  async deleteCommentHandler(req, res, next) {
    try {
      ...
      await deleteCommentUseCase.execute({ commentId: req.params.commentId }, req.params.threadId, req.auth.id);
  ...
  async putCommentLikeHandler(req, res, next) {
    try {
      ...
      await toggleCommentLikeUseCase.execute({ commentId: req.params.commentId }, req.params.threadId, req.auth.id);
